...
volatile char SButton;

volatile uint32_t button_count;
//SButton will be used in main, true when both buttons pressed
...
void LED_Init(void){
  ...
  button_count = 0;             // (b) initialize count and wait for clock
  //GPIO_PORTF_DIR_R &= ~0x11;  // (c) PF4&0 input (built-in buttons)
  //GPIO_PORTF_DEN_R |= 0x11;   //     enable digital I/O 
  //GPIO_PORTF_PUR_R |= 0x11;   //     enable weak pull-up
  GPIO_PORTF_IS_R &= ~0x11;     // (d) edge-sensitive
  GPIO_PORTF_IBE_R &= ~0x11;    //     not both edges
  GPIO_PORTF_IEV_R |= 0x11;   	//      rising edge event
  GPIO_PORTF_ICR_R = 0x11;      // (e) clear flags 
  GPIO_PORTF_IM_R |= 0x11;      // (f) arm interrupts PF4 & PF0
  NVIC_PRI7_R = (NVIC_PRI7_R&0xFF00FFFF)|0x00A00000; // (g) priority 5 !!! may want to change later! 
  NVIC_EN0_R = 0x40000000;      // (h) enable interrupt 30 in NVIC
  //EnableInterrupts();         // (i) Enable global Interrupt flag (I) <- done in ESP_Init()

	SButton = 0;
}
// -----------GPIOPortF_Handler-----------
// Called if either button on the board is pressed. 
// Will print out to the screen which button was pressed. 
...
//	ST7735_FillScreen(ST7735_BLUE);
	...
	// DAC_Pause();
	unsigned sel = GetInstrument();
	
	if(GPIO_PORTF_MIS_R == 0x11){
		//printf("Both pressed.\n");
		SButton = 1;
	}
	else if(GPIO_PORTF_MIS_R == 0x01){
		//printf("SW2 (PF0) pressed.\n");
		//DAC_PlaySong((++sel)%2); <-- !!! Needs to move to state machine in main
	}
	else if(GPIO_PORTF_MIS_R == 0x10){
		//printf("SW1 (PF4) pressed.\n");
	}
	
	GPIO_PORTF_ICR_R = 0x11;      // acknowledge either flag
	
  button_count++; //not sure we care but thats okay. 
}

//------------Board_Input------------ //obsolete with interrupt handler
