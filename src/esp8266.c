...
#define SSID_NAME  "Mak_iPhone"								//!!! change for submission
#define PASSKEY    "govolslol"
// #define SSID_NAME "WLAN_Guest"
// #define PASSKEY "Greenhill2745!"
//#define SSID_NAME  "Anika"								
//#define PASSKEY    "homeworkyay"
//#define SSID_NAME  "A&M"								
//#define PASSKEY    "AnikaMason512021"
...
//================================================Anika Defined Functions==========================
//------------ESP8266_ParseRXBuff-----------
//returns char* of data returned from RXBuff
//inputs: substring to search for
//outputs: char* found string
char *ESP8266_ParseRXBuff(char* str){
...
// Outputs: none
void ESP8266_Init(uint32_t baud, bool echo){
  ESP8266_InitUART(baud,echo); // baud rate, no echo to UART0
  ESP8266_EnableRXInterrupt();
  SearchLooking = false;
  SearchFound = false;
...
// step 1: AT+RST reset module
  UART_OutString("ESP8266 Init... ");
  if(ESP8266_Reset()==0){
    UART_OutString("\n\rReset failure, could not reset\n\r"); while(1){};
  }
...
// optional step: check to see our IP address
  if(ESP8266_GetIPAddress()==0){ // data streamed to UART0, OK
    UART_OutString("GetIPAddress error, could not get IP address\n\r"); while(1){};
  }
...
// optional step:check to see other AP in area
  if(ESP8266_ListAccessPoints()==0){
    UART_OutString("ListAccessPoints, could not list access points\n\r"); while(1){};
  }
...
int ESP8266_Reset(){
	int try=MAXTRY;
  SearchStart("ready"); //AT+GMR version 0018000902						//!!! match with your board - will either be "ok" or "ready"
//  SearchStart("ok");
  while(try){
...
//  SearchStart("no change");//AT+GMR version 0018000902										//!!! change to match board
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CWMODE_CUR=%d\r\n", mode);
...
int ESP8266_JoinAccessPoint(const char* ssid, const char* password){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CWJAP_CUR=\"%s\",\"%s\"\r\n", ssid, password); //??? verify
    ESP8266SendCommand((const char*)TXBuffer);												//does not work on UT wifi but should otherwise
    ...
    if (*ptr == 'G' && *(ptr + 1) == 'E' && *(ptr + 2) == 'T') {
      if (*(ptr + 5) == '?'){ // means data to process
        char* messagePtr = strstr(ptr, "message=") + 8;
				...
				UART_OutString("Message from ESP8266: ");
				UART_OutString(messagePtr);
				UART_OutChar('\n');
      }
