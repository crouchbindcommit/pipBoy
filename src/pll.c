...
// Setup Sysick with interrupts using PIOSC CLK_SRC
void SysTick_Init(uint32_t period){
  NVIC_ST_CTRL_R = 0;         // disable SysTick during setup
  NVIC_ST_RELOAD_R = period-1;// reload value
  NVIC_ST_CURRENT_R = 0;      // any write to current clears it
  NVIC_SYS_PRI3_R = (NVIC_SYS_PRI3_R&0x00FFFFFF)|0x40000000; // priority 2 !!! higher priority than buttons!         
  NVIC_ST_CTRL_R = 0x03; // enable SysTick with core clock and interrupts
	//Use PIOSC/4 (16MHz/4 = 4MHz clock) instead of System Clock
}
//Systick Handler written in main :) 
